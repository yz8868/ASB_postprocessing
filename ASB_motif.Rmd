...
df_SPL = read.csv("./data/SPL_loci.csv")
```
...
df_TPM_SPL9 = df_TPM %>%
  inner_join(df_SPL, by = c("AGI.code" = "ID")) %>%
  unite(ID, c("AGI.code", "Name")) %>%
...
hmap_SPL_TPM = Heatmap(t(as.matrix(df_TPM_SPL9)),
...
hmap_SPL_iBAQ = Heatmap(t(as.matrix(df_iBAQ_SPL9)),
                        col = viridis(100),
                        rect_gp = gpar(col = "white", lwd = 2),
                        cluster_columns = FALSE,
                        cluster_rows = FALSE,
