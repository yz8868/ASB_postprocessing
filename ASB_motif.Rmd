...
vari_GO = enrichGO(gene = all_gene_list,
                  keyType = "TAIR",
...
```{r}
# Construct the TPM and iBAQ dataframe.

library(readxl)
df_TPM_iBAQ = read_excel("./data/41586_2020_2094_MOESM4_ESM.xlsx", sheet = "gene expression", skip = 1)
df_TPM = df_TPM_iBAQ %>%
...
SPL_list = c("AT2G47070", "AT5G43270", "AT2G33810", "AT1G53160", "AT3G15270", "AT1G69170", "AT5G18830", "AT1G02065", "AT2G42200", "AT1G27370", "AT1G27360", "AT3G60030", "AT5G50570", "AT5G50670", "AT1G20980", "AT3G57920", "AT1G76580")
```
...
hmap_SPL_TPM = Heatmap(t(as.matrix(df_TPM_SPL9)),col = brewer.pal(9, 'YlOrRd'),
                   cluster_columns = FALSE,
                   cluster_rows = FALSE,
                   column_title = "SPL9 TPM Matrix")

hmap_SPL_iBAQ = Heatmap(t(as.matrix(df_iBAQ_SPL9)),col = viridis(100),
                   cluster_columns = FALSE,
                   cluster_rows = FALSE,
                   column_title = "SPL9 iBAQ Matrix")

hmap_SPL = hmap_SPL_TPM %v% hmap_SPL_iBAQ

hmap_SPL

png("./results/SPL_heatmap.png", width = 10, height = 10, units = "in", res = 72)
hmap_SPL
dev.off()
...
column_hc_Col_TPM = hclust(dist(mat_TPM_ColASB2))
column_dend_Col_TPM = as.dendrogram(column_hc_Col_TPM)

hmap_Col_TPM = Heatmap(mat_TPM_ColASB,
                   col = brewer.pal(9, 'YlOrRd'),
                   cluster_columns = column_dend_Col_TPM,
                   cluster_rows = FALSE,
                   column_title = "gCol TPM Matrix")

mat_iBAQ_ColASB = t(as.matrix(df_iBAQ_ColASB))
mat_iBAQ_ColASB2 = df_iBAQ_ColASB %>%
  mutate_all(., ~replace_na(., 0)) %>%
  as.matrix()

column_hc_Col_iBAQ = hclust(dist(mat_iBAQ_ColASB2))
column_dend_Col_iBAQ = as.dendrogram(column_hc_Col_iBAQ)

hmap_Col_iBAQ = Heatmap(mat_iBAQ_ColASB,
                   col = viridis(100),
                   cluster_columns = column_dend_Col_iBAQ,
                   cluster_rows = FALSE,
...
hmap_Col = hmap_Col_TPM %v% hmap_Col_iBAQ

hmap_Col

png("./results/gColASB_F1a_heatmap.png", width = 40, height = 15, units = "in", res = 72)
hmap_Col
dev.off()
...
column_hc_C24_TPM = hclust(dist(mat_TPM_C24ASB2))
column_dend_C24_TPM = as.dendrogram(column_hc_C24_TPM)

hmap_C24_TPM = Heatmap(t(as.matrix(df_TPM_C24ASB)),
                   col = brewer.pal(9, 'YlOrRd'),
                   cluster_columns = column_dend_C24_TPM,
                   cluster_rows = FALSE,
...
column_hc_C24_iBAQ = hclust(dist(mat_iBAQ_C24ASB2))
column_dend_C24_iBAQ = as.dendrogram(column_hc_C24_iBAQ)

hmap_C24_iBAQ = Heatmap(t(as.matrix(df_iBAQ_C24ASB)),
                   col = viridis(100),
                   cluster_columns = column_dend_C24_iBAQ,
                   cluster_rows = FALSE,
                   column_title = "gC24 iBAQ Matrix")

hmap_C24 = hmap_C24_TPM %v% hmap_C24_iBAQ

hmap_C24

png("./results/gC24ASB_F1a_heatmap.png", width = 40, height = 15, units = "in", res = 72)
hmap_C24
dev.off()
```
